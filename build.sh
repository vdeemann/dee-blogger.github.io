...
extract_excerpt() {
    local file="$1"
    # Skip title line and get first few content lines
    local excerpt=$(tail -n +2 "$file" | grep -v '^#' | grep -v '^```' | grep -v '^
...
echo "  - Improved search functionality with dual search inputs" | head -5 | tr '\n' ' ' | sed 's/[*`#\[\]()]/./g' | sed 's/\s\+/ /g' | cut -c1-180)
...
echo "  - $total individual post pages"
echo "  - Enhanced search functionality with dual search inputs" || echo "$((i + 1))")
    ...
    # Extract excerpt from the specific file
    excerpt=$(extract_excerpt "$file")
    if [ -z "$excerpt" ]; then
...
echo "  - Minimal design without timeline visuals"
echo "  - $total individual post pages"
echo "  - Enhanced search functionality with dual search inputs" | head -5 | tr '\n' ' ' | sed 's/[*`#\[\]()]/./g' | sed 's/\s\+/ /g' | cut -c1-180)
    # Clean up and ensure it ends properly
    echo "$excerpt" | sed 's/\.\.\.*//' | sed 's/\s*$/.../'
}
